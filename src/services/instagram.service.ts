...
import { puppeteerSetup } from "../utils/crawl";

@Injectable()
export class InstagramService {
  constructor(private configService: ConfigService) {}

  async crawlUserProfile(username: string) {
    type shareDataType = {
    ...
    if (firstUrl.includes("login")) {
      ...
      const INSTAGRAM_USERNAME = this.configService.get<string>("INSTAGRAM_USERNAME");
      const INSTAGRAM_PASSWORD = this.configService.get<string>("INSTAGRAM_PASSWORD");
